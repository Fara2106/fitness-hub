...
- iOS home-indicator band (root cause CONFIRMED via on-device test): in standalone PWA, `position:fixed` elements with `bottom:0` are clamped to the safe-area and never reach the physical bottom edge, so iOS leaves the home-indicator zone uncovered and paints it with its own system background (reads black in dark / white in light — NOT paintable from the page; an opaque `<html>` background did nothing). **Fix in `index.html`: drop `bottom:0` on `#root` and give it an explicit `height: 100dvh`** (with `100vh` / `-webkit-fill-available` fallbacks). Under `viewport-fit=cover`, `dvh` INCLUDES the safe-areas, so content + the TabBar's `padding-bottom: env(safe-area-inset-bottom)` actually fill the strip. `body` gets `overflow:hidden`. Dead-end attempts (do NOT retry): TabBar padding alone, anchoring `#root` to 4 edges with `inset:0`/`bottom:0`, recoloring `<html>` (translucent `--nav-bg` or opaque `--safe-bg`). `--safe-bg` is kept only as a harmless behind-everything fallback.
