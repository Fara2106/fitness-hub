...
- iOS home-indicator band: in standalone PWA `position:fixed` (#root, TabBar) does NOT reach the physical bottom edge, so the bottom safe-area shows the root `<html>` background. Fix in `index.html`: `<html>` background = `var(--nav-bg)` (blends with the TabBar), `<body>` stays `var(--bg)`. Earlier attempts (TabBar `padding-bottom` inset, anchoring `#root` to all 4 edges) did NOT fix it because fixed elements never extend into that inset on iOS. Verify on a real iPhone after deploy + full PWA close/reopen.
