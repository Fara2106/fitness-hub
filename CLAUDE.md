...
- iOS home-indicator band: in standalone PWA `position:fixed` (#root, TabBar) does NOT reach the physical bottom edge, so the bottom safe-area shows the root `<html>` background. Fix in `index.html`: `<html>` background = `var(--safe-bg)` — an **opaque** color (`#141416`/`#f9f9fb`) that mimics the TabBar tone; `<body>` stays `var(--bg)`. IMPORTANT: it must be opaque — using the translucent `--nav-bg` over iOS's black/white canvas still reads as black/white, so the band stays visible. Earlier attempts that did NOT work: TabBar `padding-bottom` inset, anchoring `#root` to 4 edges, `<html>` = translucent `--nav-bg`. If the opaque `--safe-bg` still doesn't fix it, the band is painted by something other than `<html>` — diagnose by temporarily setting `html { background: red }` and deploying: if the band turns red it's `<html>` (tune the color), if it stays black/white it's the `<meta name="theme-color">` / iOS canvas (`color-scheme`) instead.
